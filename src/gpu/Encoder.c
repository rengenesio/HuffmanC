...
SYMBOL eof;
SYMBOL *memory_in_enc, *cuda_memory_in;
FILE_SIZE size_in;
BYTE *memory_out_enc, *cuda_memory_out;
SIZE symbols = 0;
...
int main(int argc, char *argv[]) {
	...
	openFiles(&in_enc, argv[1], "rb", &out_enc, argv[2], "wb", &cod_enc, argv[3], "wb");

//#ifdef DEBUG
	fprintf(stderr, "\nFILES:\n\nIn: %s\nOut: %s\nCB: %s\n", argv[1], argv[2], argv[3]);
//#endif
	memory_in_enc = fileToMemory(in_enc, &size_in);
//#ifdef DEBUG
	fprintf(stderr, "\n\nSIZE:\n%u bytes\n", size_in);
//#endif
...
//#endif
	...
	if(cudaMemcpy(cuda_memory_in, memory_in_enc, sizeof(SYMBOL) * size_in, cudaMemcpyHostToDevice) != cudaSuccess) { printf("Erro cudaMemcpy cuda_memory_in\n"); return -1; }
...
//#endif
	...
	codification_dec = (CODIFICATION *) malloc(sizeof(CODIFICATION) * symbols);
	if(!codification_dec) {
...
//#ifdef DEBUG
	...
	for(unsigned int i = 0 ; i < symbols ; i++)
		fprintf(stderr, "i: %u - %x (%u) %s\n", i, codification_dec[i].symbol, codification_dec[i].size, codification_dec[i].code);
//#endif

	codificationToFile(cod_enc, codification_dec, symbols, eof);
	...
	fileCompressor <<< BLOCKS_PER_GRID , THREADS_PER_BLOCK >>> (cuda_written_bits, cuda_memory_out, codification_dec, cuda_memory_in, size_in, symbols);
	memoryToFile();
	...
	for(i = 0 ; i < POWER_BITS_CODIFICATION ; i++) {
		if(frequency_total[i] == 0) {
			if(!eof_found) {
				...
				symbols++;
				frequency_total[i]++;
				memset(memory_in_enc + (size_in / SIZEOF_SYMBOL), eof, SIZEOF_SYMBOL);
...
void treeToCode (void) {
	...
	while(codes < symbols) {
		if(n->left) {
			...
			n->visited = 1;
			codification_dec[codes].size = strlenU(path);
			codification_dec[codes].symbol = n->symbol;
			codification_dec[codes].code = (unsigned char *) malloc(sizeof(unsigned char) * (codification_dec[codes].size + 1));
			if(!codification_dec[codes].code) {
				fprintf(STDOUT, "Erro ao alocar variavel 'codification[%u].code'\n", codes);
				exit(-1);
			}
			memcpy(codification_dec[codes].code, path, codification_dec[codes].size + 1);
			path[strlenU(path)-1] = '\0';
			n = pop(s);
			codes++;
		}
